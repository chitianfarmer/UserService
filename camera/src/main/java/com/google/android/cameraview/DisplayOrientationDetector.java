...
abstract class DisplayOrientationDetector {
    ...
    public DisplayOrientationDetector(Context context) {
        mOrientationEventListener = new OrientationEventListener(context) {
            ...
            public void onOrientationChanged(int orientation) {
                ...
                final int orientationTmp = Util.roundOrientation(orientation, mLastOrientation);
                final int rotation = mDisplay.getRotation();
                if(mLastOrientation != orientationTmp || mLastKnownRotation != rotation){
                    mLastOrientation = orientationTmp;
                    mLastKnownRotation = rotation;
                    dispatchOnDisplayOrientationChanged(DISPLAY_ORIENTATIONS.get(rotation), orientationTmp);
                }
            }
        };
    }
    ...
    public void enable(Display display) {
        ...
        dispatchOnDisplayOrientationChanged(DISPLAY_ORIENTATIONS.get(display.getRotation()), DISPLAY_ORIENTATIONS.get(Surface.ROTATION_0));
    }
...
//        mLastKnownDeviceOrientation = deviceOrientation;
        onDisplayOrientationChanged(displayOrientation, deviceOrientation);
    }
