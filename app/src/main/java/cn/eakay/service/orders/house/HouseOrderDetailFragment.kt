 ...
 * @fileName: HouseOrderDetailFragment
 * @author: chitian
 * @date: 2019-07-19 08:41
 * @description:
 * @org: http://www.eakay.cn (芜湖恒天易开软件科技有限公司)
 *
 */
class HouseOrderDetailFragment : BaseFragment(), HouseOrderDetailContract.View, View.OnClickListener {

    private lateinit var presenter: HouseOrderDetailPresenter
    ...
    override fun getIntentOrderType(): String? {
        return arguments?.getString(Constants.KEY_ORDER_TYPE)
    }
    ...
    override fun showSecondServiceView() {
        ll_take_order_menu.visibility = View.GONE
        ll_start_service_menu.visibility = View.GONE
        ll_service_completed.visibility = View.GONE
        ll_service_completed_need_second.visibility = View.VISIBLE
        ll_service_second_complete.visibility = View.GONE
    }
    ...
    override fun showSecondService(time: String?, address: String?) {
        if (time.isNullOrEmpty() || address.isNullOrEmpty()) {
            return
        }
        ll_second_time.visibility = View.VISIBLE
        ll_second_address.visibility = View.VISIBLE
        tv_order_second_service_time.text =
            if (time.isNullOrEmpty()) {
                ""
            } else {
                time
            }
        tv_order_second_address.text =
            if (address.isNullOrEmpty()) {
                ""
            } else {
                address
    ...
    override fun onClick(v: View?) {
        when (v?.id) {
            ...
            R.id.tv_need_second_service ->
                presenter.needSecondaryService()
